...
#### metadata:
```
pseudoaligner and transcript abundancer counter: Kallisto
peak caller: exomePeak
parts of code in starburst plot were from R package TCGAbiolinks
```
...
```
3a starburst plot (orignal version)
============
...
3b starburst plot (log fold change version)
============

```R
exp <- topTags(lrt.kd_vs_ctrl, 10000000)$table
met <- read.table('D:\\RWD\\WJZ\\exomePeak\\kd_vs_ctrl_no7_exomePeak_out\\exomePeak_output\\diff_peak.xls', 
...
# ## starburst plot
# p <- ggplot(data = volcano_normal, environment = .e,
#             aes(x = volcano_normal$logFC.x,
#                 y = volcano_normal$logFC.y,
#                 colour = volcano_normal$threshold.starburst)) +
#   geom_point()
# #p <- p + scale_shape_discrete(
# #    labels = c("Candidate Biologically Significant"),
...
# if(!is.null(significant) & circle){
#   p <- p + geom_point( data = significant,
#                        aes(x = significant$logFC.x,
#                            y = significant$logFC.y),
#                        color = "black",
#                        shape=1,
#                        size = 8,
#                        show.legend = FALSE)
...
#   if(names.fill){
#     p <- p + geom_label_repel(
#       data = significant[abs(significant$geFDR2)>20&abs(significant$meFDR2)>50,],
#       aes(x = significant$logFC.x, y =  significant$logFC.y,
#           label = significant$symbol, fill = as.factor(significant$starburst.status)),
#       size = 4, show.legend = FALSE,
#       fontface = 'bold', color = 'black',
#       box.padding = unit(0.35, "lines"),
...
# }
# 
# #pcr_star <- significant[significant$symbol %in% pcr,]
# # LABELING PCR GENES
# if(FALSE){
#   message("Adding names to genes")
#   if(names.fill){
#     p <- p + geom_label_repel(
#       data = pcr_star,
#       aes(x = pcr_star$meFDR2, y =  pcr_star$geFDR2,
#           label = pcr_star$symbol, fill = as.factor(pcr_star$starburst.status)),
#       size = 4, show.legend = FALSE,
#       fontface = 'bold', color = 'black',
#       box.padding = unit(0.35, "lines"),
#       point.padding = unit(0.3, "lines"),
...
# 
# 
# p <-  p + geom_hline(aes(yintercept = -logFC.ge.cut), colour = "black",
#                      linetype = "dashed") +
#   geom_hline(aes(yintercept = logFC.ge.cut), colour = "black",
#              linetype = "dashed") +
#   geom_vline(aes(xintercept = -logFC.me.cut), colour = "black",
#              linetype = "dashed") +
#   geom_vline(aes(xintercept = logFC.me.cut), colour = "black",
#              linetype = "dashed")  +
#   theme_bw() +
...
# write.csv(significant, 'kd_vs_ctrl_starburst_sig.csv')
# write.csv(volcano_normal, 'kd_vs_ctrl_starburst_all.csv')

# save(volcano_normal, file = 'starburst_volcano_kd_ctrl.Rdata')

# plot(volcano_normal$logFC.x, volcano_normal$logFC.y, xlim = c(-5,9), ylim = c(-6,6))
# ggplot(volcano_normal, aes(x = logFC.x, y = logFC.y)) + geom_point(shape=1) + geom_smooth(method = lm)

kd_hyper_up <- volcano_normal[volcano_normal$logFC.y>0.5&volcano_normal$FDR.y<0.05&volcano_normal$logFC.x>0.5&volcano_normal$FDR.x<0.05,]
kd_hyper_down <- volcano_normal[volcano_normal$logFC.y<(-0.5)&volcano_normal$FDR.y<0.05&volcano_normal$logFC.x>0.5&volcano_normal$FDR.x<0.05,]
kd_hypo_up <- volcano_normal[volcano_normal$logFC.y>0.5&volcano_normal$FDR.y<0.05&volcano_normal$logFC.x<(-0.5)&volcano_normal$FDR.x<0.05,]
kd_hypo_down <- volcano_normal[volcano_normal$logFC.y<(-0.5)&volcano_normal$FDR.y<0.05&volcano_normal$logFC.x<(-0.5)&volcano_normal$FDR.x<0.05,]

save(kd_hyper_up, file = 'kd_hyper_up.RData')
save(kd_hyper_down, file = 'kd_hyper_down.RData')
save(kd_hypo_up, file = 'kd_hypo_up.RData')
save(kd_hypo_down, file = 'kd_hypo_down.RData')


ggplot() +
  geom_point(data = volcano_normal, aes(logFC.x, logFC.y), colour = "black", size = 1) +
  geom_point(data = kd_hyper_up, aes(logFC.x, logFC.y), colour = "red", size = 1) +
  geom_point(data = kd_hyper_down, aes(logFC.x, logFC.y), colour = "purple", size = 1) +
  geom_point(data = kd_hypo_up, aes(logFC.x, logFC.y), colour = "blue", size = 1) +
  geom_point(data = kd_hypo_down, aes(logFC.x, logFC.y), colour = "green", size = 1) +
  geom_vline(xintercept = c(-0.5,0.5), linetype = 'dotted') +
  geom_hline(yintercept = c(-0.5,0.5), linetype = 'dotted') +
  #geom_smooth(data = volcano_normal, aes(logFC.x, logFC.y), method = glm) +
  ylim(-5,5) +
  theme(panel.background = element_blank(),
    axis.text = element_blank())

cor.test(volcano_normal$logFC.x, volcano_normal$logFC.y)

# write.csv(hyper_up, 'hyper_up.csv')
# write.csv(hyper_down, 'hyper_down.csv')
# write.csv(hypo_up, 'hypo_up.csv')
# write.csv(hypo_down, 'hypo_down.csv')
```
